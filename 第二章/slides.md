...
1887年，赫兹正在做一个划时代的实验：验证麦克斯韦预言的电磁波是否真的存在。他用一个火花隙发射器产生电磁波，另一端的接收器火花隙来检测。实验大获成功——电磁波被证实了。

但赫兹注意到一个奇怪的现象：当紫外光照射到接收器的火花隙上时，火花变得更加明亮、更容易产生。他用玻璃（挡紫外线）遮住火花隙，增强效应消失；换成石英（透紫外线），效应恢复。

赫兹如实记录了这个现象，称之为"令人惊讶且完全无法理解的"（striking and yet totally puzzling）。他没有深入追究——毕竟他的主要目标是验证电磁波，这已经是物理学史上最伟大的实验之一了。

<div v-click>

赫兹不会想到，这个"副产品"将在18年后引发一场比电磁波本身更深刻的物理革命。
...
**哈尔瓦克斯 (1888)**：赫兹发现的第二年，哈尔瓦克斯在莱比锡用锌板和验电器做了系统实验。他发现紫外光照射下，带负电的锌板会失去电荷，不带电的锌板会变成正电。结论很清楚：光把带负电的粒子从金属表面"打"了出来。

**勒纳德 (1902)**：勒纳德用可变强度的碳弧灯进行了定量实验，光强变化范围超过**一千倍**。他的发现彻底困惑了物理学界：

<div v-click>

（1）光电子的**最大动能与光强无关**——无论光多强多弱，打出的电子速度一样快。

（2）增大光强只增加光电子的**数量**（电流），不增加每个电子的能量。

（3）光电子的动能只取决于入射光的**频率**——频率越高，电子越快。

</div>

<div v-click>

这三个结果每一个都与经典电磁理论的预言**截然相反**。经典理论认为光是连续的波，波的强度越大，电子获得的能量应该越多。
...
在光的照射下，物质表面因吸收辐射能量而发射电子的现象，我们称之为**光电效应**。

实验装置的核心：在真空管中，用单色光照射金属阴极，通过可变电压测量光电子的电流和动能。
...
经典理论认为光电子获取动能是连续的，即光照强度越大，光电子的动能越大。但实验发现：遏止电压**只与频率有关**，与光强无关。增大光强只增大光电流（电子数目），不改变每个电子的最大动能。
...
光子的全部能量 $h\nu$ 一次性交给一个电子。电子用其中一部分克服金属的束缚（**功函数** $\phi$），剩下的变成动能：

$$\boxed{K_{\max} = h\nu - \phi}$$

其中 $\phi$ 是金属的**脱出功**（work function），即把一个电子从金属表面移出所需的最小能量。

这个方程一举解释了所有经典理论无法解释的现象：

<div v-click>

（1）**瞬时响应**：一个光子一次性把全部能量给一个电子，无需积累。

（2）**动能与光强无关**：每个光子的能量只取决于频率，光强只影响光子数目。

（3）**截止频率**：当 $h\nu < \phi$ 时，单个光子的能量不足以将电子打出，$\nu_0 = \phi / h$。
...
结果适得其反：密立根的实验数据**完美地验证**了 $V_s = (h/e)\nu - \phi/e$ 的线性关系。他测定 $h = 6.57 \times 10^{-27}$ erg·s，精度达到 0.5%，成为当时最精确的普朗克常数测量。

不同金属的 $V_s$-$\nu$ 曲线**斜率相同**（都等于 $h/e$），只是截距不同（反映不同的功函数 $\phi$）。这说明 $h$ 是自然界的基本常数，而非材料性质。

</div>

<div v-click>

密立根因此获得 **1923年诺贝尔物理学奖**——讽刺的是，他最初是想否定爱因斯坦的理论，结果却为它提供了最有力的实验证据。
